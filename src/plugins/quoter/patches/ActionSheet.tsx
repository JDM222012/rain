...
import { isNativeQuoteRendererAvailable } from "../lib/nativeRenderer";
...
export default () => before("openLazy", LazyActionSheet, ([component, key, msg]) => {
    // A synced enabled-state can start the plugin on a client whose Discord
    // build lacks the native SVG renderer. Do not inject a broken action.
    if (!isNativeQuoteRendererAvailable()) return;
